...
import { buttonPrimary, card, errorText, heading, input, select, subtle } from "../ui";
...
const YieldAnalytics = () => {
  ...
  const tiles = summary
    ? [
        ...
        ["D0 / mm²", summary.d0_per_mm2?.toExponential(2) ?? "n/a", "text-white"],
  ...
  return (
    <div className="flex animate-fade-up flex-col gap-5">
      <div className="flex flex-wrap items-center gap-3">
        ...
        <select value={split} onChange={(event) => setSplit(event.target.value)} className={select}>
        ...
        {pareto.data && <span className={subtle}>first {pareto.data.wafers} wafers</span>}
      </div>

      {pareto.error && <p className={errorText}>{pareto.error}</p>}
      <div className={card}>
        <h3 className="mb-2 text-sm font-semibold text-white">Yield loss by root cause</h3>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <div className={card}>
          <h3 className="mb-2 text-sm font-semibold text-white">Radial fail rate</h3>
