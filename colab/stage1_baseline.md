...
5. Train (resumable — rerun with `--resume` after a session drop, pointing
   ...
   `!uv run python -m waferdetect.perception.train --device 0`
6. Evaluate:
   `!uv run python -m waferdetect.perception.evaluate --weights runs/train/stage1_baseline/weights/best.pt`
